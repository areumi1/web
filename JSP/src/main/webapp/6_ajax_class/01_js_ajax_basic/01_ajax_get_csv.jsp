...
<head>
	...
	<script>
		...
		window.onload=function(){
			// 1. 브라우저에 따른 XMLHttpRequest생성하기. 자바스크립트에서 ajax 사용시 필요한 함수
			xmlHttp = new XMLHttpRequest();
			
			// 2. 요청에 대한 응답처리 이벤트 리스너 등록.
			xmlHttp.onreadystatechange=on_ReadyStateChange;  // 이벤트 함수 등록
			
			// 3.서버로 보낼 데이터 생성.
			// 3.데이터 생성.
			var data = "cate=book&name=kim"; // 파라메터는 & 로 구별한다.
			
			//###########################################################
			// 4. GET방식으로 데이터 보내기, 응답은 비동기로 클라이언트<->서버간의 연결 요청준비.
			xmlHttp.open("GET", "01_server.jsp?"+data, true);		//"01_server.jsp? 에 var data = "cate=book&name=kim"; 이객체를 가져감
			// 5. 실제 데이터 전송.
			xmlHttp.send(null);
			//####
